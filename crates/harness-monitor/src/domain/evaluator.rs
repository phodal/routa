...
impl Evaluator for EntrixEvaluator {
    ...
    fn evaluate(&self, input: &EvalInput) -> Result<EvalSnapshot> {
        let fitness_mode = match input.eval_mode {
            EvalMode::Fast => crate::tui::fitness::FitnessRunMode::Fast,
            EvalMode::Full | EvalMode::PostMerge | EvalMode::Runtime => {
                crate::tui::fitness::FitnessRunMode::Full
            }
        };
        let snapshot = crate::tui::fitness::run_fitness(&self.repo_root, fitness_mode)?;
...
fn convert_fitness_to_eval_snapshot(
    fs: crate::tui::fitness::FitnessSnapshot,
    input: &EvalInput,
) -> EvalSnapshot {
