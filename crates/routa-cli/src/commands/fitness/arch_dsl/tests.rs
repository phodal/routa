...
fn loads_backend_core_sample_and_executes_all_backend_core_rules() {
    ...
    assert_eq!(report.summary.passed_rule_count, 4);
...
fn builds_legacy_backend_core_suite_reports() {
    ...
    assert_eq!(boundaries.summary_status, BackendCoreSummaryStatus::Pass);
    assert_eq!(boundaries.rule_count, 3);
    assert_eq!(boundaries.failed_rule_count, 0);
    assert_eq!(boundaries.results.len(), 3);
    assert!(boundaries
        .results
        .iter()
        .all(|result| result.status == BackendCoreRuleStatus::Pass));
    assert_eq!(
    ...
    assert_eq!(cycles.summary_status, BackendCoreSummaryStatus::Pass);
    assert_eq!(cycles.rule_count, 1);
    assert_eq!(cycles.failed_rule_count, 0);
