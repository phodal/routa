...
cargo run -q -p routa-cli -- harness budget --config tools/entrix/file_budgets.json --changed-only --base "${ROUTA_FITNESS_CHANGED_BASE:-HEAD}"
cargo run -q -p routa-cli -- harness budget --config tools/entrix/file_budgets.json --changed-only --base "${ROUTA_FITNESS_CHANGED_BASE:-HEAD}" --overrides-only
```
