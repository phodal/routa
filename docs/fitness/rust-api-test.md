...
| fitness | `GET /api/fitness/runtime` | runtime status | repo context 解析、运行中/最近结果摘要、无事件时回退 latest snapshot | TODO | `src/app/api/fitness/runtime/route.ts`, `crates/routa-server/src/api/fitness.rs` |
