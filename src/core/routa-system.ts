...
import { IssueStore, InMemoryIssueStore } from "./store/issue-store";
...
export function createPgSystem(): RoutaSystem {
  ...
  const { PgIssueStore } = require("./db/pg-issue-store") as typeof import("./db/pg-issue-store");
  ...
  const issueStore = new PgIssueStore(db);
...
export function createSqliteSystem(): RoutaSystem {
  ...
  try {
    ...
    issueStore = new SqliteIssueStore(db);
