...
function getPrimaryLabel(repoSync: RepoSyncState): string {
  if (repoSync.status === "syncing") {
    return repoSync.total > 0
      ? `Syncing ${repoSync.completed}/${repoSync.total}`
      : "Syncing repositories";
  }

  if (repoSync.status === "done") {
    return `${repoSync.total} repo${repoSync.total === 1 ? "" : "s"} updated`;
  }
...
export function KanbanRepoSyncStatus({ repoSync }: KanbanRepoSyncStatusProps) {
  ...
  const progressPercent = repoSync.total > 0
    ...
    : 0;
  const primaryLabel = getPrimaryLabel(repoSync);
  ...
  return (
    <div
      className={`min-w-0 max-w-full rounded-xl border px-2.5 py-2 ${
          ...
          {secondaryLabel && (
            <div
              className={`mt-0.5 text-[10px] leading-4 ${
                repoSync.status === "error" ? "break-words" : "truncate opacity-75"
              }`}
            >
              {secondaryLabel}
            </div>
